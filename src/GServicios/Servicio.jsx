...
export default function Servicio({ title }) {
  ...
  const cargarModel = async () => {
    if (id > 0) {
      try {
        ...
        setSelectedCliente(String(resultado.cliente));
        const fechaISO = new Date(resultado.fechaDocumento).toISOString().split("T")[0];
        setFecha(fechaISO);
        setServicios(resultado.listaItems.map(item => ({
        ...
        setErrores({
          fecha: "",
          cliente: "",
          servicios: resultado.listaItems.map(() => ({
            tipoServicio: "",
            precio: "",
          })),
        });
  ...
  const agregarServicio = () => {
    setServicios([
      ...servicios,
      { tipoServicioId: "", precio: "", observacion: "" },
    ]);

    setErrores((prevErrors) => ({
      ...prevErrors,
      servicios: [...prevErrors.servicios, { tipoServicio: "", precio: "" }],
    }));
  };
  ...
  const cambiarServicio = (index, event) => {
    ...
    if (name === "tipoServicio") {
      ...
      newServicios[index] = {
        ...newServicios[index],
        tipoServicioId: tipoServicioSeleccionado ? tipoServicioSeleccionado.id : "",
      };
    } else {
